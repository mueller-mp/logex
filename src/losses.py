...
def get_loss(args, weights, train_dataset):
    if args.loss == 'ce':
        loss_fxn = torch.nn.CrossEntropyLoss(weight=weights, reduction='none')
    elif args.loss == 'focal':
        loss_fxn = torch.hub.load('adeelh/pytorch-multi-class-focal-loss', model='FocalLoss', alpha=weights, gamma=args.fl_gamma, reduction='none')
    elif args.loss == 'ldam':
