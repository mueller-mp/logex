...
def get_attention_interpolation_factor(i, timesteps, attention_interpolation_schedule):
    if attention_interpolation_schedule is None or attention_interpolation_schedule == 'none':
       return 0
    elif 'cosine' in attention_interpolation_schedule:
        part = attention_interpolation_schedule.split('_')
        if len(part) == 2:
            tilt = float(part[1])
        else:
            tilt = 1.

        return 0.5 * (1 + math.cos(math.pi * (i / len(timesteps)**tilt)))
    elif 'threshold' in attention_interpolation_schedule:
        part = attention_interpolation_schedule.split('_')
        if len(part) == 2:
        ...
        return 1. if (i / len(timesteps)) < threshold else 0.

    else:
        raise NotImplementedError(f'Attention schedule: {attention_interpolation_schedule} not supported')

def free_unet(unet):
    # pass
    # unet.set_attn_processor(AttnProcessor(), _remove_lora=False)
    ...
    def __call__(
        ...
        if self.is_xa and xa_attention_initial_interpolation_factor > 0:
            if self.store_initial_attention_map and timestep.item() in self.initial_attention_map.keys():
                ...
                wt = xa_attention_initial_interpolation_factor
        ...
        if not self.is_xa and self_attention_initial_interpolation_factor > 0:
            if self.store_initial_attention_map and timestep.item() in self.initial_attention_map.keys():
                ...
                wt = self_attention_initial_interpolation_factor
...
def apply_segmentation_postprocessing(squash_function, xa_map_segmentation_spatial):
