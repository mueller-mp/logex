...
class DDIMInversion(BasePipeline):
    ...
    def __call__(
            ...
            # null text
            null_text_steps: int = 100,
            null_text_guidance: float = 7.5,
            null_text_lr: float = 1e-2,
            null_text_epsilon: float = 1e-5,
            null_text_ddim_config=None,
    ):
        ...
        with torch.no_grad():
            ...
            x0_enc = self.vae.encode(x0).latent_dist.mean.to(device, torch_dtype)
        ...
        with self.progress_bar(total=num_inversion_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps.flip(0)):
                pivot_targets.append(latents.clone().detach())
                ...
                # compute the more noisy sample x_t -> x_t+1
        ...
        # null-text
        if null_text_steps > 0:
            print('Null-Text Optimization')
            if null_text_ddim_config is None:
                raise ValueError('Please pass the DDIM scheduler config as null_text_ddim_config')
            forward_scheduler = DDIMScheduler.from_config(null_text_ddim_config)
            forward_scheduler.set_timesteps(num_inversion_steps, device=device)

            pivot_targets.reverse()
            with torch.no_grad():
                guidance_scale = null_text_guidance
                do_classifier_free_guidance = guidance_scale > 1.0

                prompt_embeds = self._encode_prompt(prompt, device, num_images_per_prompt, do_classifier_free_guidance,
                                                    negative_prompt).to(device)
                uncond_embeds, conditional_embeds = prompt_embeds.chunk(2)
                null_texts = []
                with self.progress_bar(total=num_inversion_steps) as progress_bar:
                    for i, t in enumerate(timesteps[0:-1]):
                        # expand the latents if we are doing classifier free guidance
                        uncond_embeds_i = uncond_embeds.clone().detach()
                        optim = torch.optim.Adam([uncond_embeds_i], lr=null_text_lr)

                        pivot_target = pivot_targets[i]

                        latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
                        latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)

                        for i in range(null_text_steps):
                            out_latents = latents.detach()
                            uncond_embeds_i.requires_grad_(True)

                            # predict the noise residual
                            with torch.enable_grad():
                                prompt_embeds = torch.cat([uncond_embeds_i, conditional_embeds])
                                noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=prompt_embeds,
                                                       cross_attention_kwargs=cross_attention_kwargs, ).sample
                                ...
                                out_latents = forward_scheduler.step(noise_pred, t, out_latents,
                                                                     **extra_step_kwargs).prev_sample
                                loss = torch.nn.functional.mse_loss(pivot_target, out_latents)
                                optim.zero_grad()
                                loss.backward()
                                optim.step()
                                print(loss.item())
                                if loss.item() < null_text_epsilon + i * 2e-5:
                                    break

                        progress_bar.update()

                        latents = out_latents
                        null_texts.append(uncond_embeds_i.clone().detach())

            null_texts = torch.stack(null_texts, dim=0)
            null_text_resulting_image = self.decode_latents(latents.detach())
            null_text_resulting_image = self.numpy_to_pil(null_text_resulting_image)

            return x_inv, image, image_x0_dec, null_text_resulting_image, null_texts
        else:
