...
if __name__=="__main__":
    ...
    for i in [0]:
        ...
        for img_idx in range(0,args.num_images):
            ...
            result_folder_postfix = f"{prompt}_{'_'.join(str(t) for t in target_classes)}"
            ...
            print(confs_agg)
            confs_target=confs_agg.min(-1).values
                        # confs_target=confs_target.sum(-1)
            ...
            max_act_idx = torch.argmax(torch.FloatTensor(confs_target)).item() # use min conf of target classes as selection criterion
            ...
            plot(original_tensor, reconstructed_tensor, img_grid, 'Min conf target', confs_target,
